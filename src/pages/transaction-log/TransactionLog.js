...
// sections
import { TransactionTableToolbar, TransactionTableRow } from '../../sections/@dashboard/transaction-log';
import { YearlyWidgetSummaryV2 } from '../../sections/@dashboard/app';
//
import { mainContext } from '../../contexts/MainContext';
import useTransaction from './service/useTransaction';
...
const STATUS_OPTIONS = [
  ...
  { label: 'Non Cash', value: 'cash:ne' },
];

const TABLE_HEAD = [
  { id: '', label: 'Payment Date', align: 'center', width: 130 },
  { id: '', label: 'Order Date', align: 'center', width: 130 },
...
export default function TransactionLog() {
  ...
  const { user } = useAuth();
  const { list, remove } = useTransaction();
  ...
  const { data: tableData, isLoading } = list({
    ...
    paymentMethod: controller.paymentMethod !== 'all' ? controller.paymentMethod : '',
  ...
  const handleFilterPayment = (val) => {
    ...
    setController({
      ...
      search: controller.search || '',
      paymentMethod: val !== 'all' ? fixStatus : '',
    });
  };
  ...
  const handleSubmit = () => {
    ...
    if (filterPayment) {
      params.paymentMethod = filterPayment !== 'all' ? filterPayment : '';
    }
  ...
  const handleExport = async () => {
    ...
    if (controller.paymentMethod !== 'all') {
      params.append("paymentMethod", controller.paymentMethod);
    }
      ...
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <Card>
          <Stack
            ...
            mx={1}
          >
            <Typography variant="h6">Transaction Log</Typography>
            ...
            <Stack flexDirection={{ md: "row" }} alignItems={{ md: "center" }} gap={2} py={2.5}>
              ...
              <Box>
                <Stack flexDirection="row" gap={2}>
                  ...
                  {['owner', 'admin', 'cashierF'].includes(user?.role?.toLowerCase()) && (
                    <LoadingButton
                      title="Export"
                      variant="contained"
                      color="info"
                ...
                <CSVLink
                  filename={`Export-Transaction-${formatDate(new Date())}`}
                  separator=";"
                  data={exportData}
                  headers={headers}
                  ref={exportCsv}
          ...
          <Grid container spacing={4} mb={3}>
            <Grid item xs={12} sm={6} md={3}>
              <YearlyWidgetSummaryV2
                title="Revenue"
                subtitle=""
                total={tableData?.summary?.revenue || 0}
                type="currency"
            ...
            <Grid item xs={12} sm={6} md={3}>
              <YearlyWidgetSummaryV2
                title="Sales"
                subtitle=""
                total={tableData?.summary?.sales || 0}
                color="warning"
                type="other"
            ...
            <Grid item xs={12} sm={6} md={3}>
              <YearlyWidgetSummaryV2
                title="Cash"
                subtitle=""
                total={tableData?.paymentSummary?.cash?.revenue || 0}
                color="success"
                type="currency"
                icon={'heroicons-solid:currency-dollar'}
                isLoading={isLoading}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <YearlyWidgetSummaryV2
                title="Non Cash"
                subtitle=""
                total={tableData?.paymentSummary?.nonCash?.revenue || 0}
                color="info"
                type="currency"
                icon={'heroicons-solid:currency-dollar'}
                isLoading={isLoading}
                  ...
                  {!isLoading ? (
                    <>
                      {tableData?.docs?.map((row) => (
                        <TransactionTableRow key={row._id} row={row} onDeleteRow={() => handleDialog(row._id)} />
                      ))}
