...
export default function Failed() {
  const { id = '' } = useParams();
  const { failedById } = useService();
  const { data, isLoading } = failedById(id);
          ...
          {isLoading ? (
            ...
            <Stack spacing={2}>
              <Stack alignItems="center" spacing={1}>
                <Iconify icon="clarity:times-circle-solid" width={160} height={160} color="red" />
                <Typography variant="h6">Transaksi Gagal</Typography>
                <Typography variant="subtitle2" color="text.secondary">
                  {formatDate2(data?.payment?.createdAt)}
                </Typography>
              </Stack>
