...
export default function ExpenseData() {
  ...
  return (
    <Page title="Expense Data">
                  ...
                  {!isLoading ? (
                    <>
                      {tableData?.docs?.map((row) => (
                        <ExpenseTableRow key={row._id} row={row} onDeleteRow={() => handleDialog(row._id)} />
                      ))}
      ...
      <ModalExpenseForm open={openForm} onClose={() => setOpenForm(false)} resetPage={handleResetPage} />
