...
// sections
import { Bill, ModalSavedBill, ModalInputCustomer, ModalScanCustomer, ModalInputVoucher, ModalSplitProduct, ModalSplitTotal, ModalPayment, FinishForm } from "../../sections/@dashboard/cashier/pos";
...
export default function CashierPos() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (ctm.generalSettings?.cashBalance && ctm.existCash?.isOpen && new Date(today).getTime() > new Date(openDate).getTime()) {
    ...
    return (
        <Page title="Pos">
            <Card sx={{ padding: "2vw 0", margin: "0 5vw", boxShadow: "0 5px 20px 0 rgb(145 158 171 / 40%), 0 12px 40px -4px rgb(145 158 171 / 12%)" }}>
                    ...
                    {!ctx.isFinished && (
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={12} md={5}>
                                <Card sx={{ height: "78vh", position: "relative", borderRadius: "8px", boxShadow: "rgba(0, 0, 0, 0.1) 0px 4px 12px;", marginRight: "1vw" }}>
                                    <Stack
                                        flexDirection="row"
                                    >
                                        <Tooltip title="Local Orders" placement="top" arrow>
                                            <Button
                                                variant="contained"
                                                size="large"
                                                sx={{ boxShadow: 0, p: 0, width: 50, height: 80, overflow: "hidden", borderTopRightRadius: 0, borderBottomRightRadius: 0 }}
                                        ...
                                        <Tooltip title="Scan Customer" placement="top" arrow>
                                            <Button
                                                variant="contained"
                                                size="large"
                                                sx={{ boxShadow: 0, p: 0, width: 50, height: 80, overflow: "hidden", borderTopLeftRadius: 0, borderBottomLeftRadius: 0 }}
                                    ...
                                    >
                                        ...
                                        <Label variant="ghost" color={ctx.customerScan ? "success" : "warning"}>
                                            {ctx.customerScan ? `Sudah Scan | Total Point: ${numberWithCommas(ctx.customerPoint || 0)}` : "Tidak Scan"}
                                        </Label>
                                    </Stack>
                                    ...
                                    >
                                        ...
                                        <Tooltip title="Reset Data" placement="top" arrow>
                                            ...
                                            >
                                                <Iconify icon="mdi:reload" width={20} height={20} sx={{ mr: 0.5 }} />Reset
                                            </Button>
                                        </Tooltip>
                                    </Stack>
                                    ...
                                    <Box sx={{ position: "absolute", bottom: "0", width: "100%" }}>
                                        <Container sx={{ height: "100%", bgcolor: "white" }}>
                                            {ctx.subtotalPrice > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Subtotal</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.subtotalPrice)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.serviceCharge > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Service Charge ({ctx.serviceChargePercentage}%)</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.serviceCharge)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.tax > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Tax ({ctx.taxPercentage}%)</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.tax)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.voucherDiscPrice > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Voucher Discount</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.voucherDiscPrice)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.discountPrice > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    {/* <Typography variant="body2" ml={1}>Discount {ctx.discount ? `(${ctx.discount}% ${ctx.discountLabel})` : ""}</Typography> */}
                                                    <Typography variant="body2" ml={1}>Discount {ctx.discountLabel ? `(${ctx.discountLabel})` : ""}</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.discountPrice)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.deliveryPrice > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Delivery Fee</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.deliveryPrice)}</Typography>
                                                </Stack>
                                            )}
                                            {ctx.havePaid > 0 && (
                                                <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                    <Typography variant="body2" ml={1}>Telah Dibayar</Typography>
                                                    <Typography variant="body2" mr={1}>Rp. {numberWithCommas(ctx.havePaid)}</Typography>
                                                </Stack>
                                            )}
                                            <Stack direction="row" justifyContent="space-between" mb={0.5}>
                                                <Typography variant="subtitle2" ml={1}>TOTAL</Typography>
                                                <Typography variant="subtitle2" fontWeight="bold" mr={1}>Rp. {numberWithCommas(ctx.totalPrice)}</Typography>
                                            </Stack>
                                        </Container>
                                        <Stack
                                            flexDirection="row"
                                        ...
                                        >
                                            ...
                                            >
                                                <Menu
                                                    anchorEl={anchorSplit}
                                                    open={splitOpen}
                                                    onClose={handleCloseSplit}
                                                >
                                                    {splitType.map((item, i) => (
                                                        <MenuItem key={i} onClick={() => handleSelectSplitType(item)}>{item}</MenuItem>
                                                    ))}
                                                </Menu>
                                                {/* <Button
                                                    variant="contained"
                                                ...
                                                >
                                                    <Iconify icon="solar:dollar-linear" sx={{ mr: 1 }} width={24} height={24} />Proceed to Pay
                                                </Button>
                                            </Stack>
                                        </Stack>
                                    </Box>
            ...
            <ModalInputCustomer
                open={openInputCustomer}
                onClose={() => setOpenInputCustomer(false)}
            />

            <ModalScanCustomer
                open={openScanCustomer}
                onClose={() => setOpenScanCustomer(false)}
            />

            <ModalInputVoucher
                open={openInputVoucher}
                onClose={() => setOpenInputVoucher(false)}
            />

            <ModalSplitProduct
                open={byProduct}
                onClose={() => setByProduct(false)}
            />

            <ModalSplitTotal
                open={byTotal}
                onClose={() => setByTotal(false)}
            />

            <ModalSavedBill
                open={openSavedBill}
                onClose={handleCloseSavedBill}
            />
