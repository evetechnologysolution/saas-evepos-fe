...
export default function LibraryBanner() {
  ...
  const getData = async ({ queryKey }) => {
    ...
    try {
      const res = await axios.get(`/banner?${queryString}`);
  ...
  const handleDelete = async () => {
    ...
    if (selectedId) {
      await axios.delete(`/banner/${selectedId}`);
      queryClient.invalidateQueries('listBanner');
  ...
  return (
    <>
      ...
      <ConfirmDelete open={open} onClose={handleDialog} onDelete={handleDelete} isLoading={isLoading} />
    </>
  );
}
