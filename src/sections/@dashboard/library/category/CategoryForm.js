...
export default function CategoryForm({ isEdit, currentData }) {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                await axios.get("/categories/paginate?page=1&perPage=50").then((response) => {
                    ...
                    <Grid item xs={12} md={5}>
                        <Card sx={{ p: 3 }}>
                            ...
                            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }} gap={1}>
                                <Button variant="outlined" onClick={() => navigate(PATH_DASHBOARD.library.category)}>Cancel</Button>
                                <LoadingButton
                                    type="submit"
                                    variant="contained"
