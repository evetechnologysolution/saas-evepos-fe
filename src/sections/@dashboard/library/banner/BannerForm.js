...
export default function BannerForm({ isEdit, currentData }) {
  ...
  const NewDataSchema = Yup.object().shape({
    ...
    image: Yup.mixed().test('fileType', 'Only image files allowed', (value) => {
      if (!value) return true;
      return value.type?.startsWith('image/');
    }),
    imageMobile: Yup.mixed().test('fileType', 'Only image files allowed', (value) => {
      if (!value) return true;
      return value.type?.startsWith('image/');
    }),
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        await axios.get('/banner?page=1&perPage=10').then((response) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      if (!isEdit) {
        ...
        await axios.patch(`/banner/${currentData._id}`, formData);
      }
      reset();
      ...
      <Card sx={{ p: 3 }}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <Stack spacing={3}>
              {isEdit && <RHFTextField name="id" label="Banner ID" disabled />}
          ...
          <Grid item xs={12} md={6}>
            ...
            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }} gap={1}>
              <Button variant="outlined" onClick={() => navigate(PATH_DASHBOARD.library.banner)}>
                Cancel
              </Button>
