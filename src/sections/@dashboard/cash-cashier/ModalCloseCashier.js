...
export default function ModalCloseCashier({ open, handleClose, handleCloseCashier }) {
  ...
  if (ctx.existCash?.detail) {
    const eWallet =
      ctx.existCash?.detail?.dana +
      ctx.existCash?.detail?.ovo +
      ctx.existCash?.detail?.shopeePay +
      ctx.existCash?.detail?.qris;
    const card =
      ctx.existCash?.detail?.bri +
      ctx.existCash?.detail?.bni +
      ctx.existCash?.detail?.bca +
      ctx.existCash?.detail?.mandiri;
    nonCash = eWallet + card;
  }

  // let estimasiSaldo = 0
  // if (ctx.existCash) {
  //     // estimasiSaldo = (ctx.existCash?.cashIn + ctx.existCash?.detail?.cash) - ctx.existCash?.cashOut;
  //     estimasiSaldo = ctx.existCash?.total || 0;
  // }

  const [estimasiSaldo, setEstimasiSaldo] = useState(0);
  ...
  const errorMessage = () => {
    ...
    if (isError) return 'Amount is Required';
    return '';
  };
  ...
  const handleChecked = (e) => {
    ...
    if (e.target.checked) {
      setDifference(0);
      setAmount(estimasiSaldo);
      setAmountDisplay(estimasiSaldo);
    } else {
      setDifference(estimasiSaldo > 0 ? -estimasiSaldo : 0);
      setAmount(0);
      setAmountDisplay('');
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const getOrder = await axios.get(`/order/close-cashier?start=${ctx.existCash?.startDate}&end=${currDate}`);
  ...
  return (
    <>
      <BootstrapDialog aria-labelledby="customized-dialog-title" maxWidth="sm" fullWidth open={open}>
        ...
        <DialogContent dividers>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <Box p={2} borderRadius="8px" bgcolor="#EBEEF2">
                <Typography variant="h6" align="center">
                  Start Shift: {formatDate2(ctx.existCash?.startDate)}
                </Typography>
              </Box>
            </Grid>
            <Grid item xs={6}>
            ...
            <Grid item xs={6}>
              <Box p={2} borderRadius="8px" bgcolor="#EBEEF2">
                ...
                <Typography variant="h6" align="center">
                  Rp {numberWithCommas(estimasiSaldo)}
                </Typography>
              </Box>
            </Grid>
            <Grid item xs={6}>
              <Box p={2} borderRadius="8px" bgcolor="#EBEEF2">
                ...
                <Typography variant="h6" align="center">
                  Rp {numberWithCommas(difference)}
                </Typography>
              </Box>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle2">Total kas akhir oleh kasir</Typography>
              <Typography variant="body2">
                Tidak bisa diubah setelah tutup kas, harap masukkan nominal yang sesuai
              </Typography>
              <FormGroup sx={{ mb: 1 }}>
                <FormControlLabel control={<Checkbox checked={checked} onChange={handleChecked} />} label="Semua" />
              </FormGroup>
              <NumericFormat
                ...
                onValueChange={(values) => {
                  ...
                  setDifference(values.value > 0 ? values.value - estimasiSaldo : -estimasiSaldo);
                }}
              />
            </Grid>
            <Grid item xs={12}>
