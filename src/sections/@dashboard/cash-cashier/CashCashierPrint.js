...
const CashCashierPrint = React.forwardRef(({ content, items }, ref) => {
    ...
    return (
        <div ref={ref} className="no-break">
            ...
            {content.history.length > 0 && content.history.map((item, i) => (
                <div key={i} style={{ margin: "5px auto" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <p style={{ textAlign: "left" }}>{item.isCashOut ? "Cash Out" : "Cash In"} <br /><span style={{ fontSize: "9px", fontStyle: "italic" }}>({truncateString(item.title)})</span></p>
                        {item.isCashOut ? (
            ...
            <div style={{ margin: "5px auto" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <p>Selisih</p>
                    <p>Rp. {numberWithCommas(closeCashier?.amount ? (content?.difference || 0) : -(content.detail.cash + content.cashIn + eWallet + card) - cashOut)}</p>
                </div>
