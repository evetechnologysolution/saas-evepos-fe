...
export default function TransactionForm({ currentData }) {
      ...
      <form onSubmit={handleSubmit}>
        <Card sx={{ p: 3 }}>
                ...
                <Stack flexDirection="row" flexWrap="wrap" gap={3}>
                  ...
                  {data?.status === 'paid' && payDate && (
                    <>
                      ...
                      <FormGroup sx={{ mb: 1, display: 'none' }}>
                        <FormControlLabel
          ...
          <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }} gap={1}>
            <Button variant="outlined" onClick={() => navigate(PATH_DASHBOARD.transactionLog.root)}>
              Back
            </Button>
            <LoadingButton type="submit" variant="contained" loading={loading} sx={{ display: 'none' }}>
              Update
            </LoadingButton>
          </Stack>
        </Card>
