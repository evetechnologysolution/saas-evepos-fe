...
export default function FinishForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await axios.patch(`/orders/raw/${ctx.currentOrderID}`, { donation });
                    ...
                    </Box>
                    {
                        ctx.splitAmount > 0 && ctx.totalPrice > 0 && (
                            <>
                                ...
                                <Box width="100%" maxWidth={400} mx="auto" mt={5} sx={{ p: 3, border: "2px solid #ccc", borderRadius: "10px" }}>
                                    ...
                                    <table style={{ width: "100%", textAlign: "left" }}>
                                        <tbody>
                                            <tr>
                                                <td><Typography variant="subtitle1" mb={1}>Order ID</Typography></td>
                                                <td><Typography variant="subtitle1" mb={1}>: {ctx.displayOrderID || ctx.currentOrderID}</Typography></td>
                                            </tr>
                                            <tr>
                                                <td><Typography variant="subtitle1" mb={1}>Split Amount</Typography></td>
                                                <td><Typography variant="subtitle1" mb={1} color="primary">: Rp. {numberWithCommas(ctx.splitAmount)}</Typography></td>
                                            </tr>
                                            <tr>
                                                <td><Typography variant="subtitle1" mb={1}>Remaining</Typography></td>
                                                <td><Typography variant="subtitle1" mb={1} color="error">: Rp. {numberWithCommas(ctx.totalPrice)}</Typography></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <Box textAlign="center">
