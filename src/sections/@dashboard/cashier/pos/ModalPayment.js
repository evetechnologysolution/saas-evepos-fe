...
export default function ModalPayment(props) {
  ...
  const handleClose = () => {
    handleReset();
    if (fromOutlet) {
      // reset voucher yang diinput dari outlet
      setFromOutlet(false);
      ctx.setVoucherCode('');
  ...
  const moneyOptions = [
    5000, 10000, 20000, 50000, 100000, 150000, 200000, 250000, 300000, 350000, 400000, 450000, 500000,
  ];
  ...
  const handleCharge = async (type = 'payment') => {
    ...
    if (type === 'payment') {
      objData = {
        ...
        status: isUpdate ? (Number(ctx.havePaid) + Number(amount) === ctx.actualPrice ? 'paid' : 'half paid') : 'paid',
  ...
  return (
    <>
      <BootstrapDialog aria-labelledby="customized-dialog-title" fullWidth maxWidth="md" open={props.open}>
        <BootstrapDialogTitle
          id="customized-dialog-title"
          onClose={handleClose}
          style={{ borderBottom: '1px solid #ccc', textAlign: 'center', fontSize: 'calc(1rem + 0.8vw)' }}
        >
          Payment
        </BootstrapDialogTitle>
        <DialogContent dividers>
          <Container>
            ...
            <Grid container spacing={2} justifyContent="center" alignItems="center">
                ...
                <>
                  <Grid item xs={6}>
                    <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
                      <Typography variant="subtitle1">Voucher Code</Typography>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="subtitle1">Voucher Discount</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="subtitle1" align="right" mr={1.5}>
              ...
              <Grid item xs={6}>
                <Typography variant="subtitle1">Tanggal Bayar</Typography>
              </Grid>
              <Grid item xs={6}>
        ...
        <DialogActions>
          <Stack flexDirection="row" sx={{ mx: 1.5, width: '100%' }}>
            <LoadingButton
              size="large"
              variant="contained"
              sx={{
